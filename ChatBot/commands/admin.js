...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "add": {
          const add = ["100064763179341"] ;
             if (!add.includes(event.senderID))
             return api.sendMessage(`${global.config.PERMISSION} only has a permission to add admin.`, event.threadID, event.messageID);
        ...
        case "rm":
        case "del": {
const remove = ["100062441630008"];
             if (!remove.includes(senderID))
             return api.sendMessage(`${global.config.PERMISSION} only has a permission to remove admin.`, threadID, messageID);
const rm = ["100062441630008"];
             if (!rm.includes(senderID))
             return api.sendMessage(`${global.config.PERMISSION} only has a permission to remove admin.`, threadID, messageID);
const del = ["100062441630008"];
             if (!del.includes(senderID))
             return api.sendMessage(`${global.config.PERMISSION} only has a permission to remove admin.`, threadID, messageID);
