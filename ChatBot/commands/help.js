...
module.exports. run = function({ api, event, args, getText }) {
    ...
    const text = `\n〘 Page: ${page}/${Math.ceil(arrayInfo.length/numberOfOnePage)} 〙\n〘 ${global.config.PREFIX}help, ${global.config.PREFIX}help 2 〙\n〘ramdom facts: ${randomText[Math.floor(Math.random()*randomText.length)]}〙\n\n〘 use ${global.config.PREFIX}callad to contact dev or admins. 〙\nowner: ${global.config.OWNER}\nmade by: ${global.config.MADE}`;
    return api.sendMessage(`CMD LIST` + "" + msg + "" + text, threadID, async (error, info) => {
			if (autoUnsend) {
