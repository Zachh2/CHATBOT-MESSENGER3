...
if ((this.config.credits) != "Joshua Sy") { return api.sendMessage(`[ WARNING ] - Detecting credits modules ${this.config.name} was changed to ${this.config.credits} ok Adminbot ${global.config.BOTNAME} 😐 Stop immediately!!!`, event.threadID, event.messageID)}
    const request = require('request');
  const fs = require("fs-extra")
  const axios = require("axios")
  const { threadID, messageID, senderID, body } = event;
  if(!args[0]) return api.sendMessage('Please enter primary name!!! example: -fbcover Ulala', threadID, messageID)
  else return api.sendMessage(`🔍 You chose the main name: ${args.join(" ").toUpperCase()}\n\n(Reply this message and choose your sub name)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`🔍 You have chosen the sub-name ${event.body.toUpperCase()}\n\n(Reply this message enter your phone number)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "sdt",
          name: `fbcover`,
          author: senderID,
