	...
	switch (handleReply.type) {
	  ...
	  case "create": {
		...
		return api.sendMessage(`Processing...`, event.threadID, async (error, info) => {
		  ...
		  var imag = (await axios.get(`https://sim.ainz-project.repl.co/canvas/fbcoverv2?name=${encodeURIComponent(name_)}&id=${idchar}&subname=${encodeURIComponent(subname_)}&color=${encodeURIComponent(color_)}`, {
			responseType: "stream"
		  })).data;
		  var msg = {
...
//https://api.phamvandien.xyz/fbcover/v2?