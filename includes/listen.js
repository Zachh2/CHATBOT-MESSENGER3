module.exports = function({ api, models }) {
  ...
  const checkttDataPath = __dirname + '/../ChatBot/commands/checktt/';
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '===Top 10 Day Interaction===\n';
     ...
     checkttData.forEach(async(checkttFile) => {
          ...
          let checkttBody = '===Top 10 Interactions of the Weekn===\n';
          checkttBody += storage.slice(0, 10).map(item => {
            return `${count++}. ${item.name} (${item.count})`;
        }).join('\n');
          api.sendMessage(checkttBody, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
(async function () {
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../ChatBot/commands/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../ChatBot/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../ChatBot/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../ChatBot/commands/cache/${a.fileName}`)) : "");
		}
...
return (event) => {
    let unsend = __dirname + "/../ChatBot/commands/cache/unsendReaction.json";
